...
public class Utils {
     ...
     * @return
     */
    public static String getPrimarykeyID(String MethodName){
        String begin=MethodName;
    ...
    public static synchronized String getDocID(){
        String begin="doc";
        String s = UUID.randomUUID().toString();
        //去掉“-”符号
        return begin+s.substring(0,8)+s.substring(9,13)+s.substring(14,18)+s.substring(19,23)+s.substring(24);
...
//        }
    ...
    //判断传入的时间是否超24小时
    public  static boolean checktime24(Date dateString){
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

        Calendar calendar = Calendar.getInstance();
//        calendar.add(Calendar.DATE, -1);// 日期减1
        Date resultDate = calendar.getTime(); // 结果
        int va = resultDate.compareTo(dateString);
        if(va>1){
            return  true;
        }else{
            return  false;
    ...
    public static String getOrderIdByTime() {
      ...
      String newDate=sdf.format(new Date());
       ...
       for(int i=0;i<3;i++){
         ...
         return newDate+result+NumberUtil.getId();
        }
    ...
    public static String getPrepareIdByUUId() {
        Date date = new Date();
        DateFormat format = new SimpleDateFormat("yyyyMMddHHmmss");
        String time = format.format(date);
        int hashCodeV = UUID.randomUUID().toString().hashCode();
        ...
        // 4 代表长度为4
        // d 代表参数为正数型
        return time + String.format("%08d", hashCodeV);
    }
